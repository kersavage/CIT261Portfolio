...
export default class QuakesController {
    ...
    if (this.position.lat === 0) {
      try {
        ...
        let gL = await getLocation();
          this.position.lat = gL.coords.latitude;
        //if we get the location back then set the latitude and longitude into this.position
        this.position.lon = gL.coords.longitude;
      } catch (error) {
        console.log(error);
      }
    }
  ...
  async getQuakesByRadius(radius = 100) {
    ...
    // get the list of quakes in the specified radius of the location
        var quakeList = new Quake();
          let qL = await quakeList.getEarthQuakesByRadius(this.position, radius);

 

    // render the list to html
    this.quakesView.renderQuakeList(qL, this.parentElement);
